...
def normalizar_item(item: str, remover_numeros: bool = REMOVER_NUMEROS) -> str:
    """lowercase, sem acentos, opcionalmente sem números, sem símbolos extras, espaços colapsados."""
    ...
    if remover_numeros:
        s = re.sub(r"[0-9]+", "", s)
    # remove pontuação/símbolos (mantém letras, dígitos e espaços)
    s = re.sub(r"[^\w\s]", " ", s)
    # underscores podem aparecer por \w; substitui por espaço
    s = s.replace("_", " ")
